...
import { CANOPY_GITHUB_URL } from "@/lib/github-repo";
...
type CanopyLandingPageProps = {
	githubStars?: number | null;
};

export function CanopyLandingPage({ githubStars = null }: CanopyLandingPageProps) {
	const t = useTranslations("landing");
	const locale = useLocale();
	const starsFormatted =
		typeof githubStars === "number"
			? new Intl.NumberFormat(locale === "en" ? "en" : "pl", {
			...
			<header className="border-border/40 bg-background/80 relative z-10 border-b border-dashed backdrop-blur-md">
				<div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-4 sm:gap-4 sm:px-6">
					...
					<div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
						<LocaleSwitcher />
						<a
							href={CANOPY_GITHUB_URL}
							target="_blank"
							rel="noopener noreferrer"
							className="border-input bg-background/90 text-foreground hover:bg-muted/70 inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-medium shadow-sm transition-colors"
						...
						>
							...
							{starsFormatted !== null ? (
								<span className="flex items-center gap-1 tabular-nums">
									<Star
										className="size-3 shrink-0 fill-amber-400/35 text-amber-600 dark:text-amber-400"
										aria-hidden
									/>
									{starsFormatted}
								</span>
							) : null}
							<span className="text-muted-foreground max-[420px]:sr-only sm:inline">
								{t("navGithub")}
							</span>
						</a>
